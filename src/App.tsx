...
function HomePage({ questions, results }: { questions: Question[]; results: TrainingResult[] }) {
  ...
  return <div className="page home-page">
    ...
    <button className="start-card" onClick={randomStart}><span className="start-icon"><Sparkles /></span><span><strong>ランダム演習を始める</strong><small>全48問から10問を出題</small></span><span className="arrow">→</span></button>
...
function HistoryPage({ questions, results }: { questions: Question[]; results: TrainingResult[] }) {
  ...
  const visible = results.filter((result) => caseFilter === 'all' || questionMap.get(result.questionId)?.case === caseFilter)
  return <div className="page"><div className="page-heading"><p className="eyebrow">LEARNING LOG</p><h1>学習履歴</h1><p>{results.length}回の答案思考を記録しています。</p></div><div className="chips compact"><Chip active={caseFilter === 'all'} onClick={() => setCaseFilter('all')}>すべて</Chip>{['I', 'II', 'III', 'IV'].map((c) => <Chip key={c} active={caseFilter === c} onClick={() => setCaseFilter(c)}>事例 {c}</Chip>)}</div>
    <div className="history-list">{visible.length ? visible.map((result) => { const q = questionMap.get(result.questionId); return <article key={result.id}><div className="history-date">{new Date(result.answeredAt).toLocaleDateString('ja-JP', { month: 'short', day: 'numeric' })}<small>{new Date(result.answeredAt).toLocaleTimeString('ja-JP', { hour: '2-digit', minute: '2-digit' })}</small></div><div><p>{q ? `${q.year} 事例${q.case} ${q.questionNo}` : result.questionId}</p><strong>{q?.questionSummary ?? '問題'}</strong><small>自己評価 {result.selfRating} {result.needsReview && '・要復習'}</small></div><div className={result.totalScore >= 70 ? 'history-score good' : 'history-score'}>{result.totalScore}<small>点</small></div></article> }) : <div className="empty-inline"><History /><p>条件に合う履歴はありません</p></div>}</div>
  </div>
}
...
function SettingsPage({ settings, setSettings, onImported, onCleared }: { settings: AppSettings; setSettings: (s: AppSettings) => void; onImported: () => Promise<void>; onCleared: () => Promise<void> }) {
  ...
  return <div className="page"><div className="page-heading"><p className="eyebrow">PREFERENCES</p><h1>設定</h1><p>自分の学習スタイルに合わせます。</p></div>
    ...
    <button className="danger-button" onClick={() => void clear()}>全履歴を削除</button><p className="version">果トレ MVP v0.1.0 ・ 問題データ 48件</p>
  </div>
}
